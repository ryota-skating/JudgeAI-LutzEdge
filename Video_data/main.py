...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == "train":
            ...
            if opt.refine:
                output_3D[:, :, 0, :] = 0
                action_error_sum_refine = test_calculation(
                    output_3D, out_target, action, action_error_sum_refine, opt.dataset, subject
                )
...
if __name__ == "__main__":
    ...
    if opt.train:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            train_data, batch_size=opt.batch_size, shuffle=True, num_workers=int(opt.workers), pin_memory=True
        )

    test_data = Fusion(opt=opt, train=False, dataset=dataset, root_path=root_path)
    test_dataloader = torch.utils.data.DataLoader(
        test_data, batch_size=opt.batch_size, shuffle=False, num_workers=int(opt.workers), pin_memory=True
    )
        ...
        if opt.train and p1 < opt.previous_best_threshold:
            ...
            if opt.refine:
                opt.previous_refine_name = save_model(
                    opt.previous_refine_name, opt.checkpoint, epoch, p1, model["refine"], "refine"
                )
